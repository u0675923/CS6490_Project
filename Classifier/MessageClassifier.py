...
class SpamClassifier:
    ...
    def appendToTrainer(self, messageDict):
        ...
        # Early stop training if validation decreases for 3 epochs in a row
        earlyStop = EarlyStopping(monitor = "val_loss", patience = 3)
        # Train on data
        self.model.fit(messageTensor, labelTensor, epochs = 20, batch_size = 1, validation_data=(self.xValidateTensor, self.yValidateTensor), callbacks=[earlyStop])
        # Save the Keras model to a directory
        self.model.save("Datasets/nnModel.h5")
...
sc = SpamClassifier()
# # We are hoping to acheive [1 0 1 1 1 2]
# print("Predicted classification of messages: ", sc.predictMessages(messages))

# # Form dictionary with correct label mapping
# appendDict = {messages[0] : 1, messages[1] : 0, messages[2] : 1, messages[3] : 1, messages[4]: 1, messages[5] : 2}
